...
self.addEventListener("fetch", event => {
  ...
  if (!isPwaPath && !isGlobalAsset && !isImage) {
    return; // Dejar pasar otras peticiones (ej. dashboard administrativo)
  }
  ...
  event.respondWith(
    fetch(req)
      .then(res => {
        // Si la respuesta es válida (status 200), hacemos una copia en caché
        // Nota: res.type === 'basic' asegura que sea del mismo origen (tu servidor)
      ...
      .catch(() => {
        ...
        return caches.match(req, { ignoreSearch: false }).then(match => {
          ...
          // 5. FALLBACK (PLAN B)
          // Si lo que falló fue una navegación a una página, mostramos offline.html
