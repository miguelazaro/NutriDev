...
/* -----------------------------
   Middlewares base
   (HSTS desactivado para evitar forzar HTTPS)
------------------------------ */
app.use(helmet({
...
app.use(session({
  ...
  cookie: {
    httpOnly: true,
    sameSite: 'lax',
    secure: false, // detrás de Nginx sin HTTPS directo
...
app.use((req, res, next) => {
  ...
  // alias de compatibilidad para vistas que usan messages.*
  res.locals.messages = {
...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    ...
    // 👇 Log útil para verificar que llegan eventos (incluye cuenta conectada)
...
// Health-check opcional para ver 200 en el navegador (el POST real es el que usa Stripe)
...
// Importante: ya NO pasamos manualmente error/success; vienen de res.locals
...
// Cobros (el router maneja allowIfSuccess para ?ok=1)
